import type { ProjectAgentShare } from "../../../types";
...
interface MixStackedBarChartProps {
  rows: Array<{
    key: string;
    label: string;
    mix: ProjectAgentShare[];
  }>;
}

export function MixStackedBarChart({ rows }: MixStackedBarChartProps) {
  ...
  return (
    <div className="space-y-5 py-2">
      {visibleRows.map((row) => (
        <div
          key={row.key}
          className="grid grid-cols-[minmax(110px,160px)_1fr] items-center gap-4"
        >
          <div
            className="truncate text-right text-xs font-medium text-text-secondary"
            title={row.label}
          >
            {row.label}
          </div>
